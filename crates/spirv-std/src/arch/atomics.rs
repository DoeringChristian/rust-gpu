...
#[inline]
pub unsafe fn atomic_load<N: Scalar, const SCOPE: u8, const SEMANTICS: u8>(ptr: &N) -> N {
...
#[inline]
pub unsafe fn atomic_store<N: Scalar, const SCOPE: u8, const SEMANTICS: u8>(
    ptr: &mut N,
    value: N,
) {
    asm! {
...
#[inline]
pub unsafe fn atomic_exchange<N: Scalar, const SCOPE: u8, const SEMANTICS: u8>(
    ptr: &mut N,
    value: N,
) -> N {
...
pub unsafe fn atomic_compare_exchange<
    I: Integer,
    const SCOPE: u8,
    const EQUAL_SEMANTIC: u8,
    const UNEQUAL_SEMANTIC: u8,
>(
    ptr: &mut I,
    value: I,
    comparator: I,
    ...
    asm! {
        ...
        "OpStore {old} %old",
        scope = const SCOPE,
        equal = const EQUAL_SEMANTIC,
        unequal = const UNEQUAL_SEMANTIC,
        ptr = in(reg) ptr,
        value = in(reg) &value,
